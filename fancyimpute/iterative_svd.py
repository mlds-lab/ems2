...
class IterativeSVD(Solver):
    ...
    def _converged(self, X_old, X_new, missing_mask):
        ...
        ssd = np.sum(np.abs(difference))
        old_norm_squared = (np.abs(old_missing_values)).sum()
        return (ssd / (1e-16+old_norm_squared)) < self.convergence_threshold
